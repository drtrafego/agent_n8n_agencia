...
function TemperatureIcon({ temp }: { temp: 'hot' | 'warm' | 'cold' }) {
  if (temp === 'hot') return <Flame size={11} className="text-orange-400" />;
  if (temp === 'warm') return <Zap size={11} className="text-yellow-400" />;
...
function StatsBar({ data }: { data: Record<string, Lead[]> }) {
  ...
  const stats = [
    { label: 'Total', value: totalLeads, color: 'text-zinc-100' },
    { label: 'Novos', value: novos, color: 'text-zinc-400' },
    { label: 'Qualificacao', value: `${taxaQualif}%`, color: 'text-blue-400' },
    { label: 'Agendamento', value: `${taxaAgend}%`, color: 'text-green-400' },
    { label: 'Conversao', value: `${taxaConv}%`, color: 'text-purple-400' },
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-1.5 mb-2 flex-wrap">
        {lead.nicho && (
          <span className="inline-block text-[10px] font-medium px-2 py-0.5 rounded-full bg-indigo-900/40 text-indigo-300 border border-indigo-800/30">
            {lead.nicho}
          </span>
        )}
        {followups > 0 && (
          <span className="inline-block text-[10px] font-medium px-1.5 py-0.5 rounded-full bg-amber-900/40 text-amber-300 border border-amber-800/30">
      ...
      {lead.last_message && (
        <p className="text-xs text-zinc-500 mb-2 leading-relaxed line-clamp-2">{truncate(lead.last_message, 80)}</p>
      )}
      ...
      <div className="flex items-center justify-between text-[10px] text-zinc-600">
        ...
        {lastActivity && lead.last_message_at && (
          <span className="text-zinc-700">ult. {timeAgo(lastActivity)}</span>
        )}
      </div>
    </div>
  );
