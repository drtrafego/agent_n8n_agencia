...
UPDATE_SQL = (
    ...
    "last_bot_msg_at = NOW(), "
    "observacoes_sdr = COALESCE(observacoes_sdr, '') || chr(10) || "
    "'Followup #' || (COALESCE(followup_count, 0) + 1)::text || "
    "' enviado em ' || TO_CHAR(NOW() AT TIME ZONE 'America/Sao_Paulo', 'DD/MM/YYYY HH24:MI') || '.' "
    "WHERE id = {{ $json.crm_contact_id }};"
)

MSG_NOVO = (
    '={{ ["Oi " + ($json.nome || "").split("|")[0].trim() + '
    '"! Vi que voce se interessou pelo Agente de IA. Posso te explicar rapidinho como funciona?", '
    '"Oi " + ($json.nome || "").split("|")[0].trim() + '
    '"! Ainda quer saber como um Agente de IA pode ajudar seu negocio? Estou aqui!", '
    '"E ai " + ($json.nome || "").split("|")[0].trim() + '
    '"! Tudo bem? Ficou com alguma duvida sobre o Agente de IA?"]'
    '[Math.floor(Math.random() * 3)] }}'
)

MSG_QUALIF = (
    '={{ ["Oi " + ($json.nome || "").split("|")[0].trim() + '
    '"! Estivemos conversando sobre o Agente de IA. Quer agendar uma demonstracao rapida?", '
    '"Oi " + ($json.nome || "").split("|")[0].trim() + '
    '"! Que tal agendarmos 15 minutinhos pra eu te mostrar o Agente ao vivo?", '
    '"E ai " + ($json.nome || "").split("|")[0].trim() + '
    '"! Ainda pensando sobre o Agente de IA? Posso te mandar um exemplo pratico!"]'
    '[Math.floor(Math.random() * 3)] }}'
)

MSG_OUTROS = (
    '={{ ["Oi " + ($json.nome || "").split("|")[0].trim() + '
    '"! Ficou alguma duvida sobre nosso Agente de IA? Estou aqui!", '
    '"Oi " + ($json.nome || "").split("|")[0].trim() + '
    '"! Passando pra ver se posso te ajudar com mais info sobre o Agente de IA.", '
    '"E ai " + ($json.nome || "").split("|")[0].trim() + '
    '"! Se quiser, posso te mostrar como o Agente funciona na pratica!"]'
    '[Math.floor(Math.random() * 3)] }}'
)
    ...
    "nodes": [
        ...
        },
        {
            "parameters": {
                "conditions": {
                    ...
                    "conditions": [
                        {"id": uid(), "leftValue": "={{ $json.phone }}", "rightValue": "", "operator": {"type": "string", "operation": "isNotEmpty"}}
                    ],
                    "combinator": "and"
                }
            },
        ...
        },
        {
            "parameters": {
                "assignments": {
                    "assignments": [
                        {"id": uid(), "name": "phone", "value": "={{ $json.phone }}", "type": "string"},
                        {"id": uid(), "name": "nome", "value": "={{ $json.nome }}", "type": "string"},
                        {"id": uid(), "name": "stage", "value": "={{ $json.stage }}", "type": "string"},
                        {"id": uid(), "name": "observacoes", "value": "={{ $json.observacoes_sdr }}", "type": "string"},
                        {"id": uid(), "name": "last_message", "value": "={{ $json.last_message }}", "type": "string"},
                        {"id": uid(), "name": "crm_contact_id", "value": "={{ $json.crm_contact_id }}", "type": "number"},
                        {"id": uid(), "name": "followup_count", "value": "={{ $json.followup_count }}", "type": "number"}
                    ]
                },
                "options": {}
            },
        ...
        },
        {
            "parameters": {
                "assignments": {
                    "assignments": [
                        {"id": uid(), "name": "body", "value": MSG_NOVO, "type": "string"},
                        {"id": uid(), "name": "phone", "value": "={{ $json.phone }}", "type": "string"},
                        {"id": uid(), "name": "crm_contact_id", "value": "={{ $json.crm_contact_id }}", "type": "number"},
                        {"id": uid(), "name": "followup_count", "value": "={{ $json.followup_count }}", "type": "number"}
                    ]
        ...
        },
        {
            "parameters": {
                "assignments": {
                    "assignments": [
                        {"id": uid(), "name": "body", "value": MSG_QUALIF, "type": "string"},
                        {"id": uid(), "name": "phone", "value": "={{ $json.phone }}", "type": "string"},
                        {"id": uid(), "name": "crm_contact_id", "value": "={{ $json.crm_contact_id }}", "type": "number"},
                        {"id": uid(), "name": "followup_count", "value": "={{ $json.followup_count }}", "type": "number"}
                    ]
        ...
        },
        {
            "parameters": {
                "assignments": {
                    "assignments": [
                        {"id": uid(), "name": "body", "value": MSG_OUTROS, "type": "string"},
                        {"id": uid(), "name": "phone", "value": "={{ $json.phone }}", "type": "string"},
                        {"id": uid(), "name": "crm_contact_id", "value": "={{ $json.crm_contact_id }}", "type": "number"},
                        {"id": uid(), "name": "followup_count", "value": "={{ $json.followup_count }}", "type": "number"}
                    ]
...
try:
    ...
    print("Workflow criado com UUIDs!")
    print("ID:", result.get("id"))
    print("Nome:", result.get("name"))
    print("Nodes:", len(result.get("nodes", [])))
except urllib.error.HTTPError as e:
    print("Erro HTTP %d: %s" % (e.code, e.read().decode()[:500]))
